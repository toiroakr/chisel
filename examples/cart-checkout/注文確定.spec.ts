...
const カート = c.variants("状態", {
  ...
  商品あり: c.object({
    ...
    お届け日: c.date().min(Temporal.PlainDate.from("2026-01-01")).optional(),
  }),
  確定済み: c.object({ カートID }),
});
...
const 具体例 = c.examples(注文を確定する, {
  ...
  "お届け日を指定しても同じように確定する": {
    given: {
      状態: "商品あり",
      カートID: "カート-4",
      明細: [{ 商品ID: "商品-A", 数量: 1, 単価: 500, 在庫数: 10 }],
      お届け日: Temporal.PlainDate.from("2026-01-10"),
    },
    expect: {
      result: { 結果: "確定", カートID: "カート-4", 合計金額: 500 },
      effects: [
        { 種類: "在庫引当", 商品ID: "商品-A", 数量: 1 },
        { 種類: "決済要求", カートID: "カート-4", 金額: 500, 内訳: { "商品-A": 500 } },
      ],
    },
  },
