...
```ts
const Line = c.object({
  quantity: c.int().refine(v => v.gte(1)),
  unitPrice: c.int().refine(v => v.gte(0)),
});
const Lines = c.array(Line).refine(v => v.length().gte(1));
```
